...
<ul class="flex justify-center flex-wrap gap-2 text-sm font-medium text-center">
    ...
    <li>
        <a href="{{ $user ? route('profile.public', $user->username) : url('/') }}" 
    ...
    @foreach ($categories as $cat)
        @php
            // 🛡️ Fallback: If slug is missing from the DB record, generate it from the name automatically
            $slug = $cat->slug ?: Str::slug($cat->name);
        @endphp
        <li>
            <a href="{{ $user ? route('profile.public', ['username' => $user->username, 'category' => $slug]) : url('/?category=' . $slug) }}" 
